...
int main(){
	...
	/*** test function ***/
	float cost = CPPML::MAE->get_cost(x.get(), y.get(), SIZE);
	
	// make sure that input didn't change
	for(int i = 0; i < SIZE; i++){
		if(x[i] != x_cpy[i]){
			std::cerr << "MAE->get_cost changed input array x which is not allowed!\n";
			exit(-1);
		}

		if(y[i] != y_cpy[i]){
			std::cerr << "MAE->get_cost changed input array y which is not allowed!\n";
			exit(-1);
		}
	}
	...
	CPPML::MAE->get_cost_derv(x.get(), y.get(), out.get(), SIZE);
	
	// make sure that input didn't change
	for(int i = 0; i < SIZE; i++){
		if(x[i] != x_cpy[i]){
			std::cerr << "MAE->get_cost_derv changed input array x which is not allowed!\n";
			exit(-1);
		}

		if(y[i] != y_cpy[i]){
			std::cerr << "MAE->get_cost_derv changed input array y which is not allowed!\n";
			exit(-1);
		}
	}

	/*** Test Derivative Numerically ***/
	CPPML::MAE->get_cost_derv(x.get(), y.get(), out.get(), DERV_SIZE);
	float baseCost = CPPML::MAE->get_cost(x.get(), y.get(), DERV_SIZE);

	for(int i = 0; i < DERV_SIZE; i++){
		x[i] += derv_h;
		float new_cost = CPPML::MAE->get_cost(x.get(), y.get(), DERV_SIZE);
		x[i] -= derv_h;
