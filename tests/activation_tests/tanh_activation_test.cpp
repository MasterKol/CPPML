...
int main(){
	...
	/*** test function ***/
	CPPML::TANH->f(in.get(), out.get(), SIZE);
	
	// make sure that input didn't change
	for(int i = 0; i < SIZE; i++){
		if(in[i] != in_cpy[i]){
			std::cerr << "TANH->f changed input array which is not allowed!\n";
			exit(-1);
		}
	}
	...
	CPPML::TANH->df(in.get(), out_buff.get(), out.get(), grad.get(), SIZE);

	// make sure that input didn't change
	for(int i = 0; i < SIZE; i++){
		if(in[i] != in_cpy[i]){
			std::cerr << "TANH->df changed input array which is not allowed!\n";
			exit(-1);
		}
	}
	...
	for(int i = 0; i < SIZE; i++){
		...
		if(abs(out_buff[i] - calc) > epsilon){
